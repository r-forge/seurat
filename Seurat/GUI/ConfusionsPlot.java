...
class ConfusionsPanel extends JPanel implements KeyListener, MouseListener,
	...
	public void addSelection(Point point1, Point point2) {
		...
		for (int i = 0; i < Experiments1.clusters.size(); i++) {
			for (int j = 0; j < ((Vector) Experiments1.clusters.elementAt(i))
					.size(); j++) {
				((ISelectable) ((Vector) Experiments1.clusters.elementAt(i))
		...
		for (int i = 0; i < Experiments2.clusters.size(); i++) {
			for (int j = 0; j < ((Vector) Experiments2.clusters.elementAt(i))
					.size(); j++) {
				((ISelectable) ((Vector) Experiments2.clusters.elementAt(i))
						.elementAt(j)).unselect(true);
			}
	...
	public void mouseClicked(MouseEvent e) {
		...
		for (int ii = 0; ii < Experiments1.clusters.size(); ii++) {
			for (int jj = 0; jj < ((Vector) Experiments1.clusters.elementAt(ii))
					.size(); jj++) {
				((ISelectable) ((Vector) Experiments1.clusters.elementAt(ii))
		...
		for (int ii = 0; ii < Experiments2.clusters.size(); ii++) {
			for (int jj = 0; jj < ((Vector) Experiments2.clusters.elementAt(ii))
					.size(); jj++) {
				((ISelectable) ((Vector) Experiments2.clusters.elementAt(ii))
						.elementAt(jj)).unselect(true);
			}
	...
	public void keyPressed(KeyEvent e) {
		...
		if (e.getKeyChar() == 'c') {
			...
			if (!VIEW) {
				...
				Rows = m.perRows;
				System.out
						.println("Pfad  " + calculateCriterium(Columns, Rows));
		 ...
		 * if (e.getKeyCode() == 38) {
		 * 
		 * gen = gen+0.5; } else if (e.getKeyCode() == 40) gen = gen-0.5;
		 * 
		 * System.out.println(gen);
		 * 
		 * Columns = new Vector(); Rows = new Vector();
		 * 
		 * for (int i = 0; i < matrix.length; i++) { Columns.add(new
		 * Integer(i)); }
		 * 
		 * 
		 * 
		 * for (int i = 0; i < matrix [0].length; i++) { Rows.add(new
		 * Integer(i)); }
		 * 
		 * 
		 * Matrix1 m = new Matrix1(Columns,Rows); Columns = m.perColumns; Rows =
		 * m.perRows;
		 * 
		 * 
		 * 
	...
	public void paint(Graphics g) {
		...
		//	System.out.println("-->>" + Experiments2.Names.elementAt(i));
			...
			int len = getLength(Experiments2.Names.elementAt(i), this
					.getGraphics());
		...
		for (int j = 0; j < counts[0].length; j++) {
			...
			g.setColor(Color.black);
			String s = cutLabels(Experiments2.Names
					.elementAt(Rows.elementAt(j)), abstandLinks - 10, g);
		...
		for (int i = 0; i < counts.length; i++) {
			...
			String s = ""
					+ cutLabels(Experiments1.Names.elementAt(Columns
							.elementAt(i)), (width - 10) / counts.length, g);
...
*/		
	...
	class Matrix {
		...
		public Matrix(Vector<Integer> Columns, Vector<Integer> Rows) {
			 ...
			 * for (int i = 0; i < ClusterColumns.size(); i++) {
			 * 
			 * Vector colCluster = new Vector(); Vector rowCluster = new
			 * Vector();
			 * sortInClusters(ClusterColumns.elementAt(i),ClusterRows.elementAt
			 * (i), colCluster, rowCluster, gen/2);
			 * 
			 * Vector resColCluster = new Vector(); Vector resRowCluster = new
			 * Vector(); for (int ii = 0; ii< colCluster.size(); ii++) { for
			 * (int j =0; j < ((Vector)colCluster.elementAt(ii)).size(); j++) {
			 * resColCluster
			 * .add(((Vector)colCluster.elementAt(ii)).elementAt(j)); }
			 * 
			 * 
			 * }
			 * 
			 * 
			 * for (int ii = 0; ii< rowCluster.size(); ii++) { for (int j =0; j
			 * < ((Vector)rowCluster.elementAt(ii)).size(); j++) {
			 * resRowCluster.
			 * add(((Vector)rowCluster.elementAt(ii)).elementAt(j)); }
			 * 
			 * 
			 * }
			 * 
			 ...
			 * 
			 * 
			 * perColumns = new Vector(); perRows = new Vector(); for (int i =
			 * 0; i < sortInClusterColumns.size(); i++) {
		...
		public void sortInClusters(Vector<Integer> columnsToSort,
				Vector<Integer> rowsToSort, Vector resColumns, Vector resRows,
				double param) {
			...
			System.out
					.println(Columns.size() + " " + Rows.size() + " " + param);
			...
			while (columnsToSort.size() != 0 && rowsToSort.size() != 0) {
				...
				for (int i = 0; i < columnsToSort.size(); i++) {
					for (int j = 0; j < rowsToSort.size(); j++) {
						if (max < matrix[columnsToSort.elementAt(i)][rowsToSort
								.elementAt(j)]) {
							max = matrix[columnsToSort.elementAt(i)][rowsToSort
									.elementAt(j)];
							maxC = i;
							maxR = j;
						}
			...
			for (int i = 0; i < columnsToSort.size(); i++) {
				...
				for (int j = 0; j < resColumns.size(); j++) {
					...
					for (int k = 0; k < clusterR.size(); k++) {
						schnittT += matrix[columnsToSort.elementAt(i)][clusterR
								.elementAt(k)];
					}
				...
				((Vector) resColumns.elementAt(col)).add(columnsToSort
						.elementAt(i));
			}
			...
			for (int i = 0; i < rowsToSort.size(); i++) {
				...
				for (int j = 0; j < resRows.size(); j++) {
					...
					for (int k = 0; k < clusterC.size(); k++) {
						schnittT += matrix[clusterC.elementAt(k)][rowsToSort
								.elementAt(i)];
					}
			...
			/*
			 * 
			 * for (int i = 0; i < resColumns.size(); i++) { Vector cols =
			 * ((Vector)resColumns.elementAt(i)); Vector rows =
			 * ((Vector)resRows.elementAt(i)); if (cols.size()<=2 || rows.size()
			 * <= 2 || param <= 1) { for (int j = 0; j < cols.size(); j++) {
			 ...
			 * 
			 * 
			 * } else sortInClusters(cols,rows,new Vector(), new Vector(),
			 * param/2);
			 * 
			 * 
			 * 
				...
				for (int i = 0; i < ClusterRows.size(); i++) {
					...
					for (int r = 0; r < clusterR.size(); r++) {
						for (int c = 0; c < clusterC2.size(); c++) {
							a++;
							schnittT += matrix[clusterC2.elementAt(c)][clusterR
									.elementAt(r)];
						}
					}
					...
					for (int r = 0; r < clusterR2.size(); r++) {
						for (int c = 0; c < clusterC.size(); c++) {
							a++;
							schnittT += matrix[clusterC.elementAt(c)][clusterR2
									.elementAt(r)];
						}
					}
		...
		public void newCluster() {
			...
			/*
			 * Integer maxCol = Columns.elementAt(maxC); Integer maxRow =
			 * Rows.elementAt(maxR);
			 * 
			 * 
			 * 
			 ...
			 * 
			 * 
			 * Columns.remove(maxC); Rows.remove(maxR);
			 * 
			 * / for (int i = 0; i < Columns.size(); i++) { if (max/gen < matrix
			 * [Columns.elementAt(i)][maxRow]) {
			 * 
			 * // System.out.print(Columns.elementAt(i) + " ");
			 * ClusterColumns.lastElement().add(Columns.elementAt(i)); } else
			 * restColumns.add(Columns.elementAt(i));
			 * 
			 * }
			 * 
			 * // System.out.println();
			 * 
			 * 
			 * for (int i = 0; i < Rows.size(); i++) { if (max/gen < matrix
			 * [maxCol][Rows.elementAt(i)]) {
			 * 
			 * // System.out.print(Rows.elementAt(i));
			 * ClusterRows.lastElement().add(Rows.elementAt(i)); } else
			 * restRows.add(Rows.elementAt(i));
			 * 
			 * }
			 */
			...
			if (Columns.size() != 0 && Rows.size() != 0)
				...
				for (int i = 0; i < Columns.size(); i++) {
					...
					for (int j = 0; j < ClusterColumns.size(); j++) {
						Vector<Integer> clusterC = (Vector) ClusterColumns
								.elementAt(j);
						Vector<Integer> clusterR = (Vector) ClusterRows
								.elementAt(j);
						double schnittT = 0;
						for (int k = 0; k < clusterR.size(); k++) {
							schnittT += matrix[Columns.elementAt(i)][clusterR
									.elementAt(k)];
						}
						schnittT /= clusterR.size();
						if (schnitt < schnittT) {
				...
				for (int i = 0; i < Rows.size(); i++) {
					...
					for (int j = 0; j < ClusterRows.size(); j++) {
						...
						double schnittT = 0;
						for (int k = 0; k < clusterC.size(); k++) {
							schnittT += matrix[clusterC.elementAt(k)][Rows
									.elementAt(i)];
						}
						schnittT /= clusterC.size();
						if (schnitt < schnittT) {
		 ...
		 * 
		 * 
		 * public void searchNew(Vector<Integer>ClusterColumns ,Vector<Integer>
		 * ClusterRows, int max) {
		 * 
		 * 
		 * 
		 ...
		 * 
		 * 
		 * Integer col = TempColumns.elementAt(0); Vector newRestRows = new
		 * Vector();
		 * 
		 * for (int i = Rows.size()-1; i>=0; i--) { if (max/gen < matrix
		 * [col][Rows.elementAt(i)]) {
		 * 
		 * TempRows.add(Rows.elementAt(i)); } else
		 * newRestRows.add(Rows.elementAt(i));
		 * 
		 * }
		 * 
		 * 
		 * Rows = newRestRows;
		 * 
		 * ClusterColumns.add(col); TempColumns.remove(0);
		 * 
		 * searchNew(ClusterColumns,ClusterRows, max); return;
		 * 
		 * }
		 * 
		 * 
		 ...
		 * 
		 * 
		 * for (int i = Columns.size()-1; i >= 0; i--) { if (max/gen < matrix
		 * [Columns.elementAt(i)][row]) {
		 * 
		 * TempColumns.add(Columns.elementAt(i)); } else
		 * newRestColumns.add(Columns.elementAt(i));
		 * 
		 * }
		 * 
		...
		public void searchNew(Vector<Integer> ClusterColumns,
				Vector<Integer> ClusterRows) {
			...
			for (int i = 0; i < Columns.size(); i++) {
				double crit = calculateCriterium(ClusterColumns, Columns
						.elementAt(i), ClusterRows);
				if (maxColCrit < crit) {
					maxColCrit = crit;
					maxCol = i;
				}
			...
			for (int i = 0; i < Rows.size(); i++) {
				double crit = calculateCriterium(ClusterColumns, ClusterRows,
						Rows.elementAt(i));
				if (maxRowCrit < crit) {
					maxRowCrit = crit;
					maxRow = i;
			...
			if (criterium >= maxColCrit && criterium >= maxRowCrit)
				return;
		...
		public double calculateCriterium(Vector<Integer> ClusterColumns,
			...
			for (int i = 0; i < ClusterColumns.size(); i++) {
				for (int j = 0; j < ClusterRows.size(); j++) {
					crit += matrix[ClusterColumns.elementAt(i)][ClusterRows
							.elementAt(j)];
				}
		...
		public double calculateCriterium(Vector<Integer> ClusterColumns,
				Integer col, Vector<Integer> ClusterRows) {
			double crit = 0;
			for (int i = 0; i < ClusterColumns.size(); i++) {
				for (int j = 0; j < ClusterRows.size(); j++) {
					crit += matrix[ClusterColumns.elementAt(i)][ClusterRows
		...
		public double calculateCriterium(Vector<Integer> ClusterColumns,
				Vector<Integer> ClusterRows, Integer row) {
			double crit = 0;
			for (int i = 0; i < ClusterColumns.size(); i++) {
				for (int j = 0; j < ClusterRows.size(); j++) {
					crit += matrix[ClusterColumns.elementAt(i)][ClusterRows
	...
	class Matrix1 {
		...
		public Matrix1(Vector<Integer> Columns, Vector<Integer> Rows) {
			...
			if (sortMethod == 1 || sortMethod == 2) {
				...
				for (int i = 0; i < ClusterColumns.size(); i++) {
					...
					sortInClusters(ClusterColumns.elementAt(i), ClusterRows
							.elementAt(i), colCluster, rowCluster, gen / 2);

					Vector resColCluster = new Vector();
					Vector resRowCluster = new Vector();
					for (int ii = 0; ii < colCluster.size(); ii++) {
						for (int j = 0; j < ((Vector) colCluster.elementAt(ii))
								.size(); j++) {
							resColCluster.add(((Vector) colCluster
									.elementAt(ii)).elementAt(j));
						}

					}

					for (int ii = 0; ii < rowCluster.size(); ii++) {
						for (int j = 0; j < ((Vector) rowCluster.elementAt(ii))
								.size(); j++) {
							resRowCluster.add(((Vector) rowCluster
									.elementAt(ii)).elementAt(j));
						}
		...
		public void searchNew(Vector<Integer> ClusterColumns,
				Vector<Integer> ClusterRows, int max) {

			if (TempColumns.size() == 0 && TempRows.size() == 0)
				return;
	...
	class Matrix3 {
		...
		public Matrix3(Vector<Integer> Columns, Vector<Integer> Rows,
				boolean optimum) {
	 ...
	 */
		...
		public double calculateCriterium2(Vector<Integer> ClusterColumns,
				Vector<Integer> ClusterRows) {
			double crit = 0;
			...
			for (int i = 0; i < ClusterColumns.size() - 1; i++) {
				for (int j = i + 1; j < ClusterColumns.size(); j++) {
					for (int row = 0; row < ClusterRows.size(); row++) {
						int a = matrix[ClusterColumns.elementAt(i)][ClusterRows
								.elementAt(row)];
						int b = matrix[ClusterColumns.elementAt(j)][ClusterRows
								.elementAt(row)];
			...
			for (int i = 0; i < ClusterRows.size() - 1; i++) {
				for (int j = i + 1; j < ClusterRows.size(); j++) {
					for (int col = 0; col < ClusterColumns.size(); col++) {
						int a = matrix[ClusterColumns.elementAt(col)][ClusterRows
								.elementAt(i)];
						int b = matrix[ClusterColumns.elementAt(col)][ClusterRows
								.elementAt(j)];
		...
		public double calculateCriterium3(Vector<Integer> ClusterColumns,
				Vector<Integer> ClusterRows) {
			double crit = 0;

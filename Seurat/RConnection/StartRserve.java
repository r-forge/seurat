...
class StreamHog1 extends Thread {
	...
	boolean capture;
	String installPath;
	StreamHog1(InputStream is, boolean capture) {
		this.is = is;
...
public class StartRserve {
	...
	public static boolean launchRserve(String cmd, String rargs, String rsrvargs, boolean debug) {
		try {
			...
			StreamHog1 errorHog = new StreamHog1(p.getErrorStream(), false);
			StreamHog1 outputHog = new StreamHog1(p.getInputStream(), false);
	...
	public static boolean checkLocalRserve() {
		...
		if (osname != null && osname.length() >= 7 && osname.substring(0,7).equals("Windows")) {
			...
			try {
				...
				StreamHog1 regHog = new StreamHog1(rp.getInputStream(), true);
				rp.waitFor();
				regHog.join();
